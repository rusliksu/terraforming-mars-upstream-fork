...
describe('Player', () => {
  ...
  describe('surrender action', () => {
    ...
    it('is offered as a standard action in multiplayer games', () => {
      const [, player] = testGame(2);

      expect(findSurrenderAction(player)).is.not.undefined;
    });

    it('is not offered in solo games or to the final active player', () => {
      const [, soloPlayer] = testGame(1);
      expect(findSurrenderAction(soloPlayer)).is.undefined;
