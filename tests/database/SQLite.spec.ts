...
class TestSQLite extends SQLite implements ITestDatabase {
  ...
  public async status(gameId: GameId): Promise<Status> {
    const rows = await this.all('SELECT DISTINCT status FROM games WHERE game_id = ? ORDER BY save_id DESC LIMIT 1', [gameId]);
  ...
  async completedTime(gameId: GameId): Promise<number | undefined> {
    const row = await this.get('SELECT completed_time FROM completed_game WHERE game_id = $1', [gameId]);
    return row.completed_time;
  }

  setCompletedTime(gameId: GameId, timestampSeconds: number): Promise<RunResult> {
    return this.run('UPDATE completed_game SET completed_time = to_timestamp(?) WHERE game_id = ?', [timestampSeconds, gameId]);
  }
}
